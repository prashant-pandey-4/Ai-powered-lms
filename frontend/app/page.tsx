    ...
    <div className="flex min-h-screen w-full flex-col" style={{ backgroundColor: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-2.5">
            ...
            {isLoaded && isSignedIn ? (
              <div className="flex items-center gap-2.5">
                <Link href="/dashboard">
                  <button className="rounded-lg px-4 py-2 text-xs font-bold text-white transition-all hover:opacity-90" style={{ backgroundColor: '#f97316' }}>
                    My Learning
                  </button>
                </Link>
                <UserButton />
              ...
              <div className="flex items-center gap-2">
                <SignInButton mode="modal">
                  <button className="rounded-lg px-4 py-2 text-xs font-semibold transition-colors" style={{ color: 'var(--text)', border: '1px solid var(--border)' }}>
                    Sign In
                  </button>
                </SignInButton>
                <SignUpButton mode="modal">
                  <button className="rounded-lg px-4 py-2 text-xs font-bold text-white transition-all" style={{ backgroundColor: 'var(--text)' }}>
                    Get Started <ArrowUpRight className="inline h-3 w-3 ml-0.5" />
                  </button>
                </SignUpButton>
              </div>
            ) : null}
      ...
      <section className="relative w-full overflow-hidden">
        <div className="mx-auto max-w-6xl px-6 pt-16 pb-8 lg:pt-24 lg:pb-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            ...
            {/* Right: Floating Anime Character (NO BOX) */}
      ...
      <section className="w-full py-16">
        <div className="mx-auto max-w-6xl px-6 space-y-8">
              ...
              {courses.slice(0, 6).map((course: any) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-4 space-y-2">
                    ...
                    <div className="mt-auto pt-2 flex items-center justify-between text-[11px]" style={{ borderTop: '1px solid var(--border)' }}>
                      <span className="font-bold text-[#f97316]">Free</span>
                      <span style={{ color: 'var(--text-muted)' }}>{course._count?.lectures || 0} Lessons</span>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          {!loading && courses.length === 0 && (
            <div className="flex flex-col items-center justify-center rounded-2xl p-16 text-center" style={{ border: '1px dashed var(--border)', backgroundColor: 'var(--bg-card)' }}>
              <BookOpen className="mb-3 h-8 w-8" style={{ color: 'var(--text-subtle)' }} />
              <h3 className="text-sm font-bold">No courses yet</h3>
              <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>Check back soon or create one in Admin Studio.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="w-full py-16" style={{ borderTop: '1px solid var(--border)' }}>
        <div className="mx-auto max-w-6xl px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            {/* Left: Floating Mentor Character */}
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-2xl p-5 space-y-3" style={{ border: '1px solid var(--border)', backgroundColor: 'var(--bg-card)' }}>
                ...
                <div className="rounded-xl p-3 text-xs" style={{ backgroundColor: 'color-mix(in srgb, var(--text) 5%, transparent)' }}>
                  <p className="font-semibold" style={{ color: 'var(--text)' }}>Q: How does QuickSort partitioning work?</p>
                </div>
                <div className="flex gap-2.5">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#f97316] text-white text-[10px] font-bold mt-0.5">AI</div>
                  <p className="text-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                    Choose a pivot, reorder so smaller elements go left and greater go right, then recursively partition sub-arrays. Average: O(N log N).
                  </p>
                </div>
              </div>
            </div>
