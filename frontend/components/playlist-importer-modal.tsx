...
export function PlaylistImporterModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-app bg-card shadow-2xl flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-app p-5">
          ...
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-lg text-muted hover:bg-[#23232a] hover:text-white"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
            ...
            <div className="space-y-4 rounded-xl border border-app bg-app p-4">
              <div className="flex items-center justify-between border-b border-app pb-3">
                <div>
                  ...
                  <p className="text-[11px] text-muted">
                    Found <span className="text-[#f97316] font-bold">{previewData.videoCount}</span> video lessons ready for import
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                />
                <label htmlFor="freeFirst" className="text-xs text-muted">
              ...
              <div className="max-h-60 space-y-2 overflow-y-auto pr-1">
                {previewData.videos?.map((v: any) => (
                  <div
                    key={v.videoId}
                    className="flex items-center justify-between rounded-lg border border-app bg-card p-2.5 text-xs"
                  >
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-muted shrink-0 ml-2">
        ...
        {/* Footer Actions */}
        <div className="flex items-center justify-end gap-3 border-t border-app p-4 bg-card">
          <button
            type="button"
            onClick={onClose}
            className="rounded-full border border-app bg-[#1c1c22] px-5 py-2 text-xs font-bold text-white hover:bg-[#23232a]"
          >
            Cancel
          </button>
          <button
            ...
            className="flex items-center gap-2 rounded-full glow-amber-btn px-6 py-2.5 text-xs font-bold text-white transition-all disabled:opacity-50"
          >
            {importing ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" />
